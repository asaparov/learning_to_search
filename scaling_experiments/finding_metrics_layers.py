...
def process_directories(base_dir, epoch, metric, seeds):
    ...
    expected_layers = [6, 12, 16, 20]  # Define the expected layers
    ...
    for seed in seeds:
        ...
        for file in os.listdir(seed_dir):
            ...
            if file.startswith("scaling_n_c_1e-5_seed") and re.search(r"_nlayers_\d+\.out$", file):
...
def main():
    ...
    ## Define the layers you want to analyze.
    layers = [6, 8, 12, 16, 20]  # Define the layers

    max_infinity = np.inf
    min_infinity = -np.inf

    # Add a random number to the output file name
    output_file = "average_metrics_layers_" + str(random.randint(1, 1000000)) + ".txt"  # Specify the output file name
