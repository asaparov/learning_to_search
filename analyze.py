...
def do_evaluate_model(filepath, star_distribution=False, max_backtrack_distance=None):
    ...
    if task == 'dfs':
        ...
        for frontier_size, branch_size in frontier_branches:
            generator.set_seed(get_seed(1))
            inputs,outputs,labels,_ = generator.generate_si_training_set(max_input_size, NUM_TEST_SAMPLES, reserved_inputs, frontier_size, branch_size, False, True, 1.0, 0)
            test_acc,test_loss,predictions = evaluate_model(model, inputs, outputs)
            confidence_int = binomial_confidence_int(test_acc, NUM_TEST_SAMPLES)
            predictions = np.array(predictions.cpu())
            '''print("Mistaken inputs:")
...
def do_evaluate_inference_model(filepath, star_distribution=False, max_backtrack_distance=None):
    model, _, _, _ = torch.load(filepath, map_location=device)
    ...
    for frontier_size, branch_size in frontier_branches:
        ...
        inputs, outputs, labels, _ = generator.generate_si_training_set(
            max_input_size, NUM_TEST_SAMPLES, reserved_inputs,
            frontier_size, branch_size, False, True, 1.0, 2)

        inputs, outputs, labels, _ = generator.generate_si_training_set(max_input_size, NUM_TEST_SAMPLES,
                                                                        reserved_inputs, frontier_size, branch_size,
                                                                        False, True, 1.0, 0)
        test_acc, test_loss, predictions = evaluate_model(model, inputs, outputs)
        confidence_int = binomial_confidence_int(test_acc, NUM_TEST_SAMPLES)
        predictions = np.array(predictions.cpu())
        ...
        print("(%u,%u) Test Inference Accuracy = %.2f±%.2f, Loss = %.6f" % (frontier_size, branch_size, test_acc, confidence_int, test_loss))
        test_accuracies.append((test_acc, confidence_int, test_loss))
...
if __name__ == "__main__":
    ...
    if os.path.isfile(filepath):
        if "inf" in filepath:
            test_accuracies = do_evaluate_inference_model(filepath)
        else:
